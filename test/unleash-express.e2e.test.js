...
test('should return the persisted results across requests of the same client', async t => {
    ...
    await request
        ...
        .expect(
            'set-cookie',
            `unleash=${cookieValue({ 'feature.variants.A': { name: 'variant_1' } })}; Path=/`
        )
        .expect('variant_1');
...
test('should return different persisted results across requests from different clients', async t => {
    ...
    return request
        ...
        .expect(
            'set-cookie',
            `unleash=${cookieValue({ 'feature.variants.A': { name: 'control' } })}; Path=/`
        )
        .expect('control');
});
...
test('should send only one set-cookie header', async t => {
    ...
    await request
        ...
        .expect(
            'set-cookie',
            `unleash=${cookieValue({ 'feature.A': true, 'feature.B': true })}; Path=/`
        );
});
