...
test('should use the resulting state of the features', t => {
    ...
    app.use((req) => {
        ...
        t.true(req.unleash.results['feature.A']);
        t.false(req.unleash.results['feature.B']);
    });
});
...
test('should use the experiment result', t => {
    ...
    app.use((req) => {
        ...
        t.true(req.unleash.results['feature.variants.A'].name === 'control');
    });
});
...
test('should use the client state of a feature even if persisted', t => {
    ...
    app.use((req) => {
        req.unleash.isEnabled('feature.A');
        t.false(req.unleash.results['feature.A']);
    });
});
...
test('should override the result of an experiment even if persisted', t => {
    ...
    app.use((req) => {
        req.unleash.experiment('feature.variants.A');
        t.true(typeof req.unleash.results['feature.variants.A'] === 'undefined');
    });
});
